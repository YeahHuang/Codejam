...
int main(){
    ...
    fto(iq, 2, 21)
       ...
       vector<pair<int, int> > final; 
        ...
        if (nmflag){
            fto(i, 0, final.size()){
                int ty = final[i].second; //这里WA了一次 swap都能写错 下次写一个函数吧
                final[i].second = final[i].first;
                final[i].first = ty;
            };
            int tmp = n;
            n = m;
        ...
        if (final.empty()==false && check(final, m, n) == false){
                cout<<"ERROR in"<<m<<" "<<n<<endl;
                printRoute(final);
                return 0;
        } else{
            if (final.size() == 0)
                cout << "Case #" << m <<","<<n << ": IMPOSSIBLE"<<endl;
            else{
                cout << "Case #" <<  m <<","<<n  << ": POSSIBLE"<<endl;
